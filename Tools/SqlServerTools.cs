...
public class SqlServerTools : ISqlServerTools
{
    ...
    public async Task<string> GetDatabaseObjectsByType(string connectionName = "DefaultConnection", string? schema = null, string objectType = "ALL")
    {
        ...
        catch (Exception ex) when (!(ex is OperationCanceledException || ex is TimeoutException))
        {
         ...
         /// </summary>
    [McpServerTool(Name = "mssql_get_agent_jobs"), Description("Gets SQL Server Agent job metadata (jobs, status, owner, etc.) from msdb.")]
    public async Task<string> GetSqlServerAgentJobs(string connectionName = "DefaultConnection")
    {
        ...
        catch (Exception ex)
        {
         ...
         /// </summary>
    [McpServerTool(Name = "mssql_get_agent_job_details"), Description("Gets detailed information (steps, schedules, history) for a specific SQL Server Agent job.")]
